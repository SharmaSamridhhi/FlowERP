...
Every PR and push to `main` runs [`.github/workflows/ci.yml`](.github/workflows/ci.yml) (lint, type-check, test with coverage, build) — see [FLO-010](specs/FLO-010-ci-pipeline.md). Recommended: turn on branch protection on `main` requiring this check to pass before merging — a GitHub repository setting, not something in this codebase, left for the user to enable.
