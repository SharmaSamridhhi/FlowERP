...
- **Addendum (added during FLO-016):** the insufficient-stock `ConflictError` above only carried a human-readable message initially — FLO-016 needed structured detail to map the error onto a specific line item in the UI rather than parsing the message string. `ConflictError` gained an optional `details` param (mirroring `ValidationError`'s), and `confirmChallan` now also throws with an `InsufficientStockItem[]` detail array (`productId`, `productName`, `requestedQuantity`, `availableQuantity` — new shared schema in `sales-challan.schema.ts`). See FLO-016's own Implementation Notes for the full rationale.
